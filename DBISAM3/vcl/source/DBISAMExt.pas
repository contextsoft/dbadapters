...
{$ENDIF}
  ...
  Db, DBISAMTb, DBISAMCn, dbExtParser, dbExtUtils,
...
type
  ...
  {:! and TDBISAMQueryExt. }
  TDBISAMDatabaseExt = class(TDBISAMDatabase, ICtxDataProvider, ICtxDatabase, ISchemaDatabase, IDatabaseExt)
  protected
...
begin
  ...
  if Result = nil then
  begin
    Result := TDBISAMTableExt.Create(Self); // MB: 09.26.2007 Fixing memory leak
...
begin
  if FCachedTables = nil then exit; // MB: 09.26.2007 Fixing a/v on exit
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          FldDef.Assign(Table.FieldDefs[J]); // dbschema
          FldDef.Description := Table.FieldDefs[J].DisplayName;
        end;
...
function DBExecute(DBQuery: TDBISAMQuery; const SQL: string;
  Params: TParams = nil; Query: TDBISAMQuery = nil): Integer;
var
  TempQuery: Boolean;
begin
  ...
  try
    Query.Active := False;
    Query.SessionName := DBQuery.SessionName;
    Query.DatabaseName := DBQuery.DatabaseName;
    Query.SQL.Text := SQL;
    if (Params <> nil) then
...
function TDBISAMQueryExt.PSExecuteStatement(const ASQL: string; AParams: TParams;
...
begin
  ...
  begin
    ...
    try
      Result := DBExecute(Self, ASQL, AParams, TDBISAMQuery(ResultSet^));
    finally
      with TDBISAMQuery(ResultSet^) do
        if (StmtHandle = nil) or (StmtHandle.StatementType <> SELECT_STATEMENT) then
          FreeAndNil(ResultSet^);
    end;
  end else
