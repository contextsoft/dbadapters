...
begin
   ...
   if SHGetPathFromIDList(ItemIDList,@Buffer) then
      begin
      Result:=True;
      DirectoryStr:=StrPas(PChar(@Buffer));
      end;
end;
...
begin
  ...
  begin
    ...
    if DBISAMDatabase.Session.RemotePort <> DBISAMCn.DEFAULT_PORT then
...
begin
  ...
  begin
    ...
    begin
      ...
      DBISAMDatabase.Session.RemotePort := StrToIntDef(RemotePort, DBISAMCn.DEFAULT_PORT);
    end;
    ...
    if rbRemote.Checked then begin
      ...
      begin
        ...
        begin
          RemotePort := copy(RemoteHost, P+1, MaxInt);
          RemoteHost := copy(RemoteHost, 1, P - 1);
          DBISAMSession.RemotePort := StrToIntDef(RemotePort, DBISAMCn.DEFAULT_PORT);
        end;

        DBISAMSession.RemoteHost := RemoteHost;
        {$IFDEF DBISAM_V4}
        if cbxRemoteType.ItemIndex = 0 then
