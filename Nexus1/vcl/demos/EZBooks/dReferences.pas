...
uses
  ...
  Db, dbEnum, dbSchemaEnum, nxdb, nxdbext;
...
(*
  ...
  in order to be able to replace Tnxxxx components with some other
...
type
  TdmReferences = class(TDataModule)
    tblTaxTypes: TnxTableExt;
    enOrderStatuses: TDBSchemaEnum;
    tblItems: TnxTableExt;
    tblCustomers: TnxTableExt;
    tblFormOfPayments: TnxTableExt;
    tblOrderTotals: TnxTableExt;
    tblOrders: TnxTableExt;
    tblCounters: TnxTableExt;
...
begin
  ...
  begin
    ...
    try
      ...
      Post;
      Result := Result + TnxDatabaseExt(Database).SnapshotID * 10000000;
    except
      Cancel;
      raise;
    end;
...
begin
  //frmEZBooksMain.nxEvents.HookUpToDataSets(Self);
